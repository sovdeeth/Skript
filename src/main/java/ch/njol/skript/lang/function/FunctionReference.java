...
public class FunctionReference<T> implements Contract, Executable<Event, T[]> {
	...
	public boolean validateFunction(boolean first) {
		...
		// set parameter types
		parameterTypes = Arrays.stream(sign.getParameters())
				.map(p -> p.type.getC())
				.toArray(Class<?>[]::new);
	...
	private void getRawParameterTypes() {
	...
	private String stringified() {
		...
		return "%s(%s)".formatted(functionName, args);
	}
	...
	private Signature<?> getRegisteredSignature() {
		Retrieval<Signature<?>> attempt = FunctionRegistry.getRegistry().getSignature(script, functionName, parameters);
		if (attempt.result() == RetrievalResult.EXACT) {
			return attempt.retrieved()[0];
		}
	...
	private Function<?> getRegisteredFunction() {
		Retrieval<Function<?>> attempt = FunctionRegistry.getRegistry().getFunction(script, functionName, parameters);
		if (attempt.result() == RetrievalResult.EXACT) {
			return attempt.retrieved()[0];
		}
