...
public class HTMLGenerator {
	...
	private static final Comparator<? super SyntaxElementInfo<?>> annotatedComparator = (o1, o2) -> {
		...
		if (o1.elementClass.getAnnotation(NoDoc.class) != null) {
			if (o2.elementClass.getAnnotation(NoDoc.class) != null)
				return 0;
			return 1;
		} else if (o2.elementClass.getAnnotation(NoDoc.class) != null)
			return -1;

		Name name1 = o1.elementClass.getAnnotation(Name.class);
		Name name2 = o2.elementClass.getAnnotation(Name.class);
		if (name1 == null)
			throw new SkriptAPIException("Name annotation expected: " + o1.elementClass);
		if (name2 == null)
			throw new SkriptAPIException("Name annotation expected: " + o2.elementClass);
	...
	private static <T> Iterator<SyntaxElementInfo<? extends T>> sortedAnnotatedIterator(Iterator<SyntaxElementInfo<? extends T>> it) {
		...
		while (it.hasNext()) {
			...
			if (item.elementClass.getAnnotation(Name.class) == null && item.elementClass.getAnnotation(NoDoc.class) == null) {
				Skript.warning("Skipped generating '" + item.elementClass + "' class due to missing Name annotation");
				continue;
			}
			list.add(item);
		}
			...
			while (generate != -1) {
				...
				if (genType.equals("effects") || isDocsPage) {
					...
					for (Iterator<SyntaxElementInfo<? extends Section>> it = sortedAnnotatedIterator(Skript.getSections().iterator()); it.hasNext(); ) {
						...
						assert info != null;
						if (EffectSection.class.isAssignableFrom(info.elementClass)) {
							if (info.elementClass.getAnnotation(NoDoc.class) != null)
								continue;
				...
				if (genType.equals("sections") || isDocsPage) {
					for (Iterator<SyntaxElementInfo<? extends Section>> it = sortedAnnotatedIterator(Skript.getSections().iterator()); it.hasNext(); ) {
						SyntaxElementInfo<? extends Section> info = it.next();
						assert info != null;
						boolean isEffectSection = EffectSection.class.isAssignableFrom(info.elementClass);
						// exclude sections that are EffectSection from isDocsPage, they are added by the effects block above
						if ((isEffectSection && isDocsPage) || info.elementClass.getAnnotation(NoDoc.class) != null)
							continue;
	...
	private String generateAnnotated(String descTemp, SyntaxElementInfo<?> info, @Nullable String page, String type) {
		...
		String ID = docId != null ? (docId != null ? docId.value() : null) : info.elementClass.getSimpleName();
		// Fix duplicated IDs
		if (page != null) {
